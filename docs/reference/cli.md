...
| `serve --console-directory PATH` | Serve the built console assets from that directory on the actor REST listener; requires `--rest-bind`. |
...
`serve --console-directory PATH` serves the browser build of the console described in the [console guide](../guides/console.md). The option requires `--rest-bind`, because the assets are published by the actor REST listener only and are refused on the admin listener. `GET /console` returns the directory's `index.html` and `GET /console/{file}` returns one flat asset from the same directory; only `.html`, `.js`, `.css`, `.json` and `.svg` names with a simple lowercase stem are served, an asset larger than four megabytes is refused, and responses carry `cache-control: no-store`. Asset serving never reaches the daemon gateway and relaxes nothing: the listener still requires mutual TLS, so a browser must present an accepted client certificate.

`serve --telemetry-file PATH` is the only way to turn span export on; there is no environment variable, no configuration key, and no default that enables it. When the flag is present the daemon appends one OpenTelemetry OTLP/JSON record per line to that owner-only file, optionally labelled by `--telemetry-service NAME`, and flushes it before the listeners stop. Four span names are exported: `hypermind.request` for each dispatched daemon request, `hypermind.ingestion` for each appended event batch, `hypermind.extraction` for each consolidation run, and `hypermind.provider` for each structured model call. Span attributes are integers, booleans and fixed labels only: keys and string values are compile-time constants, so no memory content, query text, prompt, conversation identifier, capability token, model name, or filesystem path can appear in an exported span.
