...
`HM_CONSOLIDATION_PROVIDER=centra` separately enables provider-backed consolidation through `CENTRA_GATEWAY_URL` and `CENTRA_GATEWAY_API_KEY`, using the exact route `openrouter/openai/gpt-5.6-luna`. A key alone does not activate this path. A consolidation run has explicit limits for LLM calls, tokens, micro-USD, and wall time (`max_llm_calls`, `max_tokens`, `max_microusd`, `max_wall_ms` in the MCP budget). Set these within the authorized spend/data-transfer scope; enabling the runtime is not authorization for an unbounded run. `HM_CONSOLIDATION_PARALLEL` (default 4, clamped to 1 through 64) bounds how many consolidation candidates are extracted concurrently within one run; an unset, empty, zero, or unparsable value falls back to 4. It bounds concurrency only and is not a spend limit: the run's `max_llm_calls`, `max_tokens`, `max_microusd`, and `max_wall_ms` still bound what the run may spend. These controls do not change independent benchmark settings or turn derived memories into observed evidence.

`HM_PROVIDER_MAX_IN_FLIGHT` (default 8) bounds how many provider calls the process may have in flight at once, and `HM_PROVIDER_MAX_IN_FLIGHT_PER_ACTOR` (default 2) bounds how many of those slots a single actor may hold, so one actor cannot occupy the whole gate. `HM_PROVIDER_ADMISSION_WAIT_MS` (default 2000) is how long a call waits for a slot before it is refused; `0` means take a slot only if one is free right now. An unset, empty, or unparsable value falls back to its default. These three variables bound concurrency only and are not a spend limit: they do not change `max_llm_calls`, `max_tokens`, `max_microusd`, or `max_wall_ms`, which continue to bound a consolidation run exactly as before. A refusal from this gate is reported as `kCapacityExceeded`.
