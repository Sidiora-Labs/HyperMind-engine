...
func Dial(cfg Config) (*Client, error) {
	if (cfg.SocketPath == "") == (cfg.GRPCTarget == "") {
		return nil, fmt.Errorf("%w: select exactly one SocketPath or GRPCTarget", ErrProtocol)
	}
...
func (c *Client) connectLocked() error {
	...
	for {
		if c.cfg.GRPCTarget != "" {
			conn, err = dialGRPC(c.cfg)
		} else {
			conn, err = net.DialTimeout("unix", c.cfg.SocketPath, time.Until(deadline))
		}
...
func (c *Client) roundTripLocked(requestID uint64, finished []byte) (*protocol.Response, []byte, error) {
	if err := writeFrame(c.conn, finished, c.cfg.RequestTimeout); err != nil {
		c.dropConnLocked()
		return nil, nil, fmt.Errorf("%w: write: %w", ErrUnavailable, err)
	}
	for {
		payload, err := c.reader.next(c.cfg.RequestTimeout)
		if err != nil {
			c.dropConnLocked()
			return nil, nil, fmt.Errorf("%w: read: %w", ErrUnavailable, err)
		}
		envelope := protocol.GetRootAsWireEnvelope(payload, 0)
...
func responseError(response *protocol.Response) error {
	...
	if response.Payload(&table) &&
		...
		return &EngineError{
			EffectState: effectState(detail.EffectState()),
			Code:        detail.Code(),
...
func (c *Client) sendAdmittedLocked(clientSeq uint64, send func(clientSeq uint64) error) error {
	...
	for resolution := 0; resolution < 2; resolution++ {
		...
		if errors.As(err, &transportErr) && transportErr.EffectState == "not_dispatched" {
			engineRejected = true
		}
