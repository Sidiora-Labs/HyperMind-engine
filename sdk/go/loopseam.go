...
func wrapEnvelope(event AppendEvent, connectionID [16]byte, clientSeq uint64,
	...
	schema.EventEnvelopeAddSchemaVersion(builder, 2)
	authority := schema.Authorityuser_asserted
	switch event.Kind {
	case KindDeliveredMsg, KindReasoning, KindToolCall:
		authority = schema.Authorityassistant_generated
	case KindToolResult:
		authority = schema.Authoritytool_observed
	case KindProviderFrame:
		authority = schema.Authorityexternal_observed
	case KindConsolidation:
		authority = schema.Authorityderived_inference
	}
	schema.EventEnvelopeAddAuthority(builder, authority)
...
func buildAssertion(builder *flatbuffers.Builder, assertion Assertion) flatbuffers.UOffsetT {
	...
	ranges := assertion.ProvenanceRanges
	if len(ranges) == 0 {
		for _, span := range assertion.Provenance {
			ranges = append(ranges, [4]uint64{span[0], span[1], 0, 0})
		}
	}
	schema.AssertionStartProvenanceVector(builder, len(ranges))
	for index := len(ranges) - 1; index >= 0; index-- {
		schema.CreateProvenanceRange(builder, ranges[index][0], ranges[index][1], uint32(ranges[index][2]), uint32(ranges[index][3]))
	}
	provenance := builder.EndVector(len(ranges))
	...
	return AppendEvent{
		...
		Build: func(builder *flatbuffers.Builder) (byte, flatbuffers.UOffsetT) {
			...
			for index := len(retraction.Provenance) - 1; index >= 0; index-- {
				schema.CreateProvenanceRange(builder, retraction.Provenance[index][0], retraction.Provenance[index][1], 0, 0)
			}
...
func ParseBundle(encoded []byte) (*Bundle, error) {
	if len(encoded) >= 4 && string(encoded[:4]) == "HMA1" {
		return parseModernBundle(encoded)
	}
...
func eventTable(kind EventKind, payload []byte) (table flatbuffers.Table, err error) {
	...
	if !ok || len(payload) < 8 ||
		!flatbuffers.BufferHasIdentifier(payload, "NCEV") {
		return flatbuffers.Table{}, ErrProtocol
	}
	envelope := schema.GetRootAsEventEnvelope(payload, 0)
	if (envelope.SchemaVersion() != 1 && envelope.SchemaVersion() != 2) || envelope.PayloadType() != expected {
		return flatbuffers.Table{}, ErrProtocol
	}
...
func RenderBundle(bundle *Bundle, premises []string) string {
	...
	if bundle.ModernSections != nil {
		return renderModernText(bundle, premises)
	}
...
func ProjectBundleExcluding(bundle *Bundle, exclusions ProjectionExclusions) []MemoryProjection {
	...
	if bundle.ModernSections != nil {
		return projectModern(bundle, exclusions)
	}
...
func (s *LoopSeam) Consolidate(ctx context.Context, assertions []Assertion) (AppendAck, error) {
	...
	for i := range assertions {
		if len(assertions[i].ProvenanceRanges) != 0 {
			continue
		}
		for _, span := range assertions[i].Provenance {
			if span[0] == 0 || span[1] < span[0] || span[1]-span[0] >= 4096 {
				return AppendAck{}, ErrProtocol
			}
			records, truncated, err := s.client.Transcript(ctx, s.conversation, span[0]-1, uint32(span[1]-span[0]+1))
			if err != nil {
			...
			for _, record := range records {
				if record.Lsn > span[1] {
					break
				}
				decoded, err := DecodeEvent(record.Kind, record.Payload)
				if err != nil {
					return AppendAck{}, err
				}
				if len(decoded.Content) != 0 {
					assertions[i].ProvenanceRanges = append(assertions[i].ProvenanceRanges, [4]uint64{record.Lsn, record.Lsn, 0, uint64(len(decoded.Content))})
				}
			}
		}
	}
